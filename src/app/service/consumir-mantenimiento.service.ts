...
//}
getPMovilidad() :Observable<any>{
  return this.http.get(url + 'pmov').pipe(map(this.extractData));
}
...
getMovsubm() :Observable<any>{
  return this.http.get(url + 'smov').pipe(map(this.extractData));
}
getConvenio() :Observable<any>{
  return this.http.get(url + 'conven').pipe(map(this.extractData));
}
